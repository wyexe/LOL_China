...
{
	static Point BlueCamp(3548.00f, 8206.00f, 51.19f);
	static Point RedCamp(11402.00f, 6532.00f, 51.70f);
...
{
	static Point BlueCamp(7488.00f, 3716.00f, 52.00f);
	static Point RedCamp(7312.00f, 11140.00f, 55.15f);
	return emCamp == em_Camp::em_Camp_Blue ? BlueCamp : RedCamp;
}

CONST Point& CGameRes::GetDroganPoint() CONST throw()
{
	static Point DroganPoint(10186.00f, 4882.00f, -71.24f);
	return DroganPoint;
}

CONST Point& CGameRes::GetBaronPoint() CONST throw()
{
	static Point BaronPoint(0, 0, 0);
	return BaronPoint;
}
...
CONST Point* CGameRes::GetNextMovePoint(_In_ em_Camp emCamp, _In_ em_Path_Type emPathType, _In_ CONST Point& CurPoint) CONST throw()
{
	...
	return Index + 1 >= Vec.size() ? &Vec.back() : &Vec.at(Index + 1);
}
...
CONST vector<Point>& CGameRes::GetPathPointVecByCampAndPathType(_In_ em_Camp emCamp, _In_ em_Path_Type emPathType) CONST throw()
{
	...
	if (TopRedCamp.size() == 0)
		std::reverse_copy(std::begin(TopBlueCamp), std::end(TopBlueCamp), TopRedCamp);
	if (MiddleRedCamp.size() == 0)
		std::reverse_copy(std::begin(MiddleBlueCamp), std::end(MiddleBlueCamp), MiddleRedCamp);
	if (ButtomRedCamp.size() == 0)
		std::reverse_copy(std::begin(ButtomBlueCamp), std::end(ButtomBlueCamp), ButtomRedCamp);
...
CONST vector<CGameRes::ResEqument>& CGameRes::GetResEuqmentVecByHero(_In_ em_Hero_Pro emHeroPro) CONST throw()
{
...
CGameRes::ResSkill& CGameRes::GetResSkillByHero(_In_ em_Hero_Pro emHeroPro)  CONST throw()
{
